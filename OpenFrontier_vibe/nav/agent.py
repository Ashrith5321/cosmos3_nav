...
class NavigationAgent:
    ...
    def __init__(
        ...
        self._lockin_replans = 0
        self._budget_fallback_done = False
        self._reverify_rejections = 0
        self._segmentation_cycles = 0
        ...
        # Frontier-conditioned world model (worldmodel/), built in setup_system
        self.world_model = None
        self.wm_config = self.config.get("world_model") or {}
        # Map-based geometric frontier supplement (frontier/geometric.py)
        self.geometric_frontier_cfg = self.config.get("geometric_frontiers") or {}
    ...
    def navigation(self, save_images: Optional[bool] = None) -> None:
        ...
        # Always STOP before the habitat env budget expires: ObjectNav only
        # scores success if STOP is called within range - an env-budget death
        # is an automatic failure even when standing on the goal (observed:
        # a continue_navigation failure at 0.0m from the goal).
        henv = getattr(self, "habitat_env", None)
        if henv is not None:
            try:
                remaining = henv._max_episode_steps - henv._elapsed_steps
                if remaining <= 3:
                    self.log(
                        "info",
                        self.logging_file,
                        f"Env budget nearly spent ({remaining} left); issuing final STOP.",
                    )
                    return False, "final_stop"
        ...
        # World model: closed-loop verification of past predictions against
        # what is actually being observed (prediction residuals + calibration)
        ...
        if (
            ...
            # Detection-recall boost: ~40% of timeout episodes never got a
            # single SAM3 proposal for the goal. Once deep into the episode
            # with zero detections, alternate the prompt with a synonym.
            SEG_SYNONYMS = {
                "tv_monitor": "television screen",
                "plant": "potted plant",
                "sofa": "couch",
                "chair": "armchair",
                "toilet": "toilet bowl",
            ...
            self._segmentation_cycles += 1
            seg_target = self.goal
            if (
                not self.detected_objects
                and self.navigation_steps > 200
                and self._segmentation_cycles % 2 == 1
            ):
                seg_target = SEG_SYNONYMS.get(self.goal, self.goal)
        ...
        if (
            ...
            # Geometric frontier supplement: map-based extraction catches
            # every opening the map knows about, including ones FrontierNet
            # never marked (view-dependence is one of its failure modes).
            if (
                self.geometric_frontier_cfg.get("enabled", False)
                and self.ft_manager.free_map is not None
            ):
                try:
                    from frontier.geometric import (
                        extract_geometric_frontiers,
                        filter_near_existing,
                    )

                    geo_fts = extract_geometric_frontiers(
                        free_pts=self.ft_manager.free_map,
                        occ_pts=self.ft_manager.occ_map,
                        voxel_size=self.VOX_SIZE,
                        nav_level=self.planner.nav_level,
                        params=self.geometric_frontier_cfg,
                    )
                    geo_fts = filter_near_existing(
                        geo_fts,
                        [f.pos3d for f in self.ft_manager.valid_frontiers],
                        min_separation=float(
                            self.geometric_frontier_cfg.get("min_separation", 1.0)
                        ),
                    )
                    if geo_fts:
                        if "snap_point" in dir(self.planner):
                            for ft in geo_fts:
                                ft.pos3d = self.planner.snap_point(ft.pos3d)
                                ft.pos3d[2] += self.C_T_R[1, 3]
                        parent = self.ft_manager.current_robot_id
                        parents = [parent] if parent is not None else None
                        self.ft_manager.add_frontiers(geo_fts, parent_ids=parents)
                        self.ft_manager.filter_frontiers()
                        geo_fts = [f for f in geo_fts if f.is_valid]
                        self.log(
                            "info",
                            self.logging_file,
                            f"Geometric frontiers added: {len(geo_fts)}",
                        )
                        if self.world_model is not None and geo_fts:
                            self.world_model.attach_context(
                                ft_list=geo_fts,
                                rgb=rgb,
                                depth=depth,
                                W_T_C2=W_T_C2,
                    ...
                    self.log(
                        "error",
                        self.logging_file,
                        f"Geometric frontier extraction failed: {e}",
                    )

            # World model: attach goal-agnostic context (local crops +
            # frozen-encoder embeddings) to the surviving new frontiers
            if self.world_model is not None and ft_list:
                try:
                    self.world_model.attach_context(
                        ft_list=ft_list,
                        rgb=rgb,
                    ...
                    self.log(
                        "error",
                        self.logging_file,
                        f"World model attach_context failed: {e}",
                    )
                            ...
                            for i, label in enumerate(labels):
                                if i < len(ft_list):
                                    ...
                                    if label in probabilities:
                                        # JSON mode guarantees parseable JSON but
                                        # not the schema: some models return the
                                        # probability quoted ("0.3") or emit a
                                        # bare value instead of [p, reason]. An
                                        # unvalidated str here survives until
                                        # merge_frontiers() does
                                        # _probabilities.mean(), which then dies
                                        # on a string dtype and kills the episode.
                                        entry = probabilities[label]
                                        ...
                                        try:
                                            ...
                                            p = 0.5
                                        ft.probability = min(max(p, 0.0), 1.0)
                                        ft.justification = str(
        ...
        # World model: coarse-to-fine predictive cascade over the top-M
        # candidates (predict beyond-frontier futures, score against the goal)
        if self.world_model is not None:
            try:
                self.world_model.ranker.set_budget_progress(
                    self.navigation_steps / max(self.max_steps, 1)
                )
                self.world_model.step(
                    current_pose=W_T_C2, step=self.navigation_steps
                )
            except Exception as e:
                self.log(
                    "error", self.logging_file, f"World model step failed: {e}"
                )
        ...
        if self.is_pointnav or (reach_next_update and self.move_enough):
            ...
            if self.ft_manager.current_goal_ft_id is not None:
                ...
                if self.world_model is not None:
                    self.world_model.notify_goal_selected(goal_frontier)
        ...
        # exploring into a guaranteed timeout (budget exhaustion was ~19% of
        # episodes; their median final distance to goal was 4.5 m).
        if (
            not self._budget_fallback_done
            and self.navigation_steps >= self.max_steps - 100
            and self.ft_manager.object_lockin is None
            and self.goal_object is None
        ):
            self._budget_fallback_done = True
            # accept unverified candidates even if flagged invalid: invalidation
            # usually came from a failed viewpoint-frontier plan, which says
            # nothing about the object itself (measured: timeouts regularly die
            # with only 'unverified/invalid' candidates on the books)
            candidates = [
                o
                for o in self.detected_objects
                if getattr(o, "verification_status", "unverified") == "unverified"
            ]
            if candidates:
                nearest = min(
                    candidates,
                    key=lambda o: np.linalg.norm(
                        np.asarray(o.centroid) - W_T_C2[:3, 3]
                    ),
                )
                self.log(
                    "info",
                    self.logging_file,
                    "Budget fallback: locking into nearest unverified candidate.",
                )
                self.ft_manager.lock_into_object(nearest)
                self.path_to_go = (
        ...
        if self.ft_manager.object_lockin is not None:
            ...
            self._lockin_steps = getattr(self, "_lockin_steps", 0) + 1
            ...
            # Anti-orbit cap: the centroid sits INSIDE the object, so the
            # stop radius can be physically unreachable and the agent circles
            # the goal until timeout (measured: 293 approach steps at 0.0m
            # from the goal). Standing next to the object beats a timeout.
            if self._lockin_steps > 60:
                self.log(
                    "info",
                    self.logging_file,
                    f"Lock-in approach exceeded 60 steps at {dist:.2f} m; stopping here.",
                )
                return False, "object_found"

            # Near-miss recovery: an exhausted path with the object still out of
            # success range used to end the episode there (a large share of
            # false positives stopped 1-2m short). Replan toward the object a
            # few times before giving up.
            if (
                len(self.path_to_go) == 0
                and dist >= self.success_threshold
                and self._lockin_replans < 4
            ):
                self._lockin_replans += 1
                self.log(
                    "info",
                    self.logging_file,
                    f"Object out of range with empty path; replanning approach "
                    f"({self._lockin_replans}/4).",
                )
                self.path_to_go = (
                    self.ft_manager.plan_path_to_goal(
                        W_T_C2, depth=depth, use_graph=False
            ...
            if len(self.path_to_go) == 0 or dist < self.success_threshold:
                # Final-viewpoint re-verification: the original verification
                # often happened from far away or an oblique angle (FP median
                # stop distance was 2.2m). Confirm from where we now stand;
                # on rejection, release the lock and resume exploring.
                if (
                    self.reverify_on_stop
                    ...
                    if prob < self.termination_threshold:
                        ...
                        self.log(
                            "info",
                            self.logging_file,
                            f"Stop-time re-verification rejected (p={prob:.2f}); "
                            f"releasing lock ({self._reverify_rejections}/3).",
                        )
                        if obj is not None:
                            obj.is_valid = False
                            obj.verification_status = "false_positive"
        ...
        if self.world_model is not None:
            self.world_model.log_step(self.navigation_steps, W_T_C2)
    ...
    def setup_system(self) -> None:
        ...
        # Frontier-conditioned world model (design: predictive frontier ranking)
        if self.wm_config.get("enabled", False):
            from worldmodel.pipeline import build_pipeline

            self.world_model = build_pipeline(
                cfg=self.wm_config,
                manager=self.ft_manager,
                goal=self.goal,
                save_dir=self.save_dir,
            )
            self.world_model.set_detector(self.ft_detector)
            self.log(
                "info",
                self.logging_file,
                f"World model enabled (backend={self.wm_config.get('backend', 'zero_shot')}).",
            )
