...
def hm3d_config(path: str = HM3D_CONFIG_PATH, stage: str = "val", episodes=200):
    ...
    with read_write(habitat_config):
        ...
        # Clamp episode subsampling to the scene's actual pool size. Habitat draws
        # num_episode_sample episodes WITHOUT replacement; requesting more than a
        # scene contains raises "Cannot take a larger sample than population" and
        # the scene is skipped entirely. Some HM3D val scenes have <28 episodes.
        n_sample = episodes
        if episodes and episodes > 0:
            ...
            scene_json = os.path.join(
                DATA_PATH, "objectnav_hm3d_v2", "val", "content", f"{stage}.json.gz"
            )
            if os.path.exists(scene_json):
                try:
                    with gzip.open(scene_json, "rt") as fh:
                        avail = len(json.load(fh).get("episodes", []))
                    if avail:
                        n_sample = min(episodes, avail)
                except Exception:
                    pass
        ...
        habitat_config.habitat.task.measurements.update(
            {
                "top_down_map": TopDownMapMeasurementConfig(
                    ...
                    draw_goal_aabbs=False,  # semantic-object AABB drawing crashes on some scene builds (cluster)
