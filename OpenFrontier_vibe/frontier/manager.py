...
class FrontierManager(Base):
    ...
    def __init__(
        ...
        # World-model hooks (worldmodel/pipeline.py). When set:
        # - external_utility_fn(valid_frontiers, current_pos) replaces the
        #   baseline utility formula in update_utility()
        # - on_frontiers_merged(kept_uid, absorbed_uids) keeps the persistent
        #   frontier memory consistent across merges
    ...
    def merge_frontiers(self):
        ...
        to_remove = []
        to_add = []
        merge_events = []  # (kept_uid, [absorbed_uids]) for the world-model memory
            ...
            if len(_fts) <= 1:
                ...
                for ft in _fts:
                    if ft.id == self.current_goal_ft_id:
                        ...
                        merge_events.append(
                            (ft.uid, [f.uid for f in _fts if f.id != ft.id])
                        )
                        self.log(
                            "info",
                            self.logging_file,
                ...
                if not merged_into_goal:
                    ...
                    # geometric frontiers carry no image anchoring; average
                    # over whichever members have one
                    _angles = [ft.direct_angle for ft in _fts if ft.direct_angle is not None]
                    _ft.direct_angle = np.average(_angles, axis=0) if _angles else None
                    _pixels = [ft.pixel_pos for ft in _fts if ft.pixel_pos is not None]
                    _ft.pixel_pos = np.average(_pixels, axis=0) if _pixels else None
                    _ft.set_valid()
                    ...
                    # persistent identity: inherit the dominant source's uid and
                    # its cached world-model features
                    _dominant = max(_fts, key=lambda f: f.u_gain or 0.0)
                    _ft.uid = _dominant.uid
                    for _key in ("wm_mean", "wm_sigma", "wm_room_top", "wm_events"):
                        if _key in _dominant.features:
                            _ft.features[_key] = _dominant.features[_key]
                    merge_events.append(
                        (_ft.uid, [f.uid for f in _fts if f.uid != _ft.uid])
                    )
                    _ft.parent_ids = list(
        ...
        if self.on_frontiers_merged is not None:
            for kept_uid, absorbed_uids in merge_events:
                if absorbed_uids:
                    self.on_frontiers_merged(kept_uid, absorbed_uids)
    ...
    def update_utility(self, current_pos) -> None:
        """
        Update each valid frontier's utility.

        Baseline (Eq.(8) in the paper):
            utility = (blended_gain ** utility_g_factor) / max(distance, 1e-6)

        When a world-model pipeline is attached (external_utility_fn set),
        utilities are the predictive Q_i scores instead (worldmodel/ranker.py).
        """
        p = np.asarray(current_pos, dtype=float).reshape(3)

        valid = self.valid_frontiers

        if self.external_utility_fn is not None:
            self.external_utility_fn(valid, p)
            self.logger.debug(
                f"World-model utility updated for {len(valid)} valid frontiers."
            )
            return
        for ft in valid:
